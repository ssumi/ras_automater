...
from NOAA_data_download_forecast_obs_no_flow import *
...
## STORM DURATION:
df3 = NOAA_forecast_obs(l_falls) 
df4 = NOAA_forecast_obs(lwt)

common_start = max(df3.index[0],df4.index[0])
t_y0 = common_start.year
t_m0 = common_start.month
t_d0 = common_start.day
t_hr0 = common_start.hour

common_end = min(df3.index[-1],df4.index[-1])
...
df3 = NOAA_forecast_obs(gage)

del df3['Stage']                                                                    #--Delete flow and date column
#del df3['Date(EST)'] 
#df3['Flow'] = df3['Flow'].map(lambda x: x.lstrip('+-').rstrip('kcfs'))             # if kcfs is not removed already
...
#idx_hr = pd.date_range(start_index, end_index, freq='{}H'.format(dd3)) 
#df3 = df3.reindex(idx_hr, fill_value = '')

##--Select the data between the range of ras time
df3 = df3[~(df3.index < common_start)]
df3 = df3[~(df3.index > common_end)]

#---Get the Stage data from whole data frame 
## Not used here: Assuming that 60% water is flowing downstream and 40% going to aqueduct                                                                       
#-----------------------------------------------------------------------------
...
df3_LF = pd.DataFrame()
gage = l_falls
df31 = NOAA_forecast_obs(gage)
df3_LF['Stage'] = df31['Stage']*0.3048 #meter
df3_litfals = df3_LF.round(3)
df3_litfals.to_csv('Little_Falls.tsv', sep='\t',encoding='utf-8' )

## WISCONSIN AVENUE 
gage = gtwn
df3_WA2 = pd.DataFrame()
df3_WA =  NOAA_forecast_obs(gage) #stage in ft
df3_WA2['Stage'] = df3_WA['Stage']*0.3048 #meter
#idx = df3_WA['Date(EST)']
#df3_WA2.index = idx
df_Wis_Ave = df3_WA2.round(3)
df_Wis_Ave.to_csv('Wisconsin_Avenue.tsv',sep='\t',encoding='utf-8')


## ALEXANDRIA
gage = alex
df3_alex2 = pd.DataFrame()
df3_alex = NOAA_forecast_obs(gage)  #stage in ft time EDT
df3_alex2['Stage'] = df3_alex['Stage']*0.3048
#idx = df3_alex['Date(EST)']  
#df3_alex2.index = idx
df_alexandria = df3_alex2.round(3)
df_alexandria.to_csv('Alexandria.tsv',sep='\t',encoding='utf-8')
...
df4 = df4[~(df4.index < df3.index[0])]   ## match index with df3
df4 = df4[~(df4.index > df3.index[-1])]  ## match index with df3
...
df41 = NOAA_forecast_obs(gage_4)
df4_lwt['Stage'] = df41['Stage']*0.3048
#idx = df4['Date(EST)']
#df4_lwt.index = idx
NOAA_lewisetta = df4_lwt.round(3)
NOAA_lewisetta.to_csv('Lewisetta.tsv',sep ='\t',encoding='utf-8') 

 
## PINEY POINT
gage = piney 
df4_pny2 = pd.DataFrame()
df4_pny = NOAA_forecast_obs_no_flow(gage)
df4_pny2['Stage'] = df4_pny['Stage']*0.3048
#idx = df4_dahl['Date(EST)']
#df4_dahl2.index = idx
NOAA_pny2 = round(df4_pny2,3)
NOAA_pny2.to_csv('Piney_Point.tsv',sep='\t',encoding='utf-8') 

  
## DAHLGREN
gage = ncd 
df4_dahl2 = pd.DataFrame()
df4_dahl = NOAA_forecast_obs(gage)
del df4_dahl['Flow']
df4_dahl2['Stage'] = df4_dahl['Stage']*0.3048
#idx = df4_dahl['Date(EST)']
#df4_dahl2.index = idx
NOAA_dahl = df4_dahl2.round(3)
NOAA_dahl.to_csv('Dahlgren.tsv',sep='\t',encoding='utf-8') 
...
gage = wadc
df4_dc = pd.DataFrame()
df4_wadc = NOAA_forecast_obs(gage)
df4_dc['Stage'] = df4_wadc['Stage']*0.3048
#idx = df4_wadc['Date(EST)']
#df4_dc.index = idx
NOAA_dc = df4_dc.round(3)
NOAA_dc.to_csv('Washington_DC.tsv',sep='\t',encoding='utf-8')
...
y_f1, m_f1, d_f1 = f1.year, f1.month, f1.day    # End date
...
idx_15T = pd.date_range(df4.index[0], df4.index[-1], freq='15T')
...
df1_BLD2.to_csv('Bladensburg.tsv', sep='\t',encoding='utf-8')

 
#------------------------------------------------------------------------------

#---------------------------UPDATED RAS DATETIME---------------------------####

#--Format datetime for HEC-RAS (insert_line)
ras_start = df4.index[0].strftime(format = '%d%b%Y')
ras_end = df4.index[-1].strftime(format = '%d%b%Y')

#--Fix the simulation time window
sim_start_hr = '%02d' % df4.index[0].hour
sim_start_min = '%02d' % df4.index[0].minute

sim_end_hr =  '%02d' % df4.index[-1].hour                                          # simulation hour in 00:00 format
sim_end_min = '%02d' % df4.index[-1].minute                                        # simulation minute in 00:00 format
