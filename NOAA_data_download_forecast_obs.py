...
def NOAA_forecast_obs(gage):
    ...
    df_obs['Flow'] = df_obs['Flow'].astype(str).str[:-4].astype(np.float)      # remove string kcfs
    df_obs['Flow'] = df_obs['Flow']*1000 # convert kcfs to cfs
    ...
    df_fcst['Flow'] = df_fcst['Flow'].astype(str).str[:-4].astype(np.float)      # remove string kcfs
    df_fcst['Flow'] = df_fcst['Flow']*1000 # convert kcfs to cfs
    ...
    if df_fcst.empty == False:
        ...
        for i in range (0,len(df_obs_new)):
            if df_obs_new['Stage'].iloc[i] == '':
                ...
                df_obs_new['Flow'].iloc[i] = df_obs_new['Flow'].iloc[i-1]
